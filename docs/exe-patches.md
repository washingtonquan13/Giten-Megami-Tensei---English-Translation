...
| dev | 0x4390C4, 0x439103, 0x43913C | `E8 <exec_token 0x439020>` | `E8 <.trc wrapper>` | **Interpreter tracer.** The wrapper snapshots the interpreter state, calls the real `exec_token`, then appends one 30-byte record to `ddswin	race.bin` behind an 8-byte `"GTRC" u16 version u16 record_size` header: `u16 file, u16 rec, u16 pc, u16 ch, i16 r, u8 capflag, u8 caplen, u16 idx_off, u16 idx_len, u16 pc0, u16 flags, u16 state, u32 rec_hash, u16 image_end, u16 handle`. **`rec_hash` and `image_end` are v4 (2026-09-10) and are what make the trace checkable against the overlay at all**: overlay v6 keys a translated span on the CONTENT of the record it lives in, so a trace carrying only the engine's file label -- written on load, not on every context switch -- names the wrong file often enough to be useless. `rec_hash` is FNV-1a over the record the interpreter is standing in, taken from the same index entry the snapshot already reads and under the same guards; `image_end` is `idx[255].off + idx[255].len`, the four bytes the hook itself reads to tell a real program counter from a virtual one. The hash loop runs only for a real script buffer (entry 0 at 0x400). **`file`, `rec`, `pc0` and the index entry are read *before* the call**; `pc` after it. That matters: the engine clears the script context when a script ends and `exec_token` is still reached once afterwards, so v1 -- which read everything afterwards -- logged `pc = 0` and `idx_off = 0` for every token that ended a script and could not place those events at all (106 of 18,683 on the traced routes). The snapshot lives in stack locals, not the cave buffer, because it spans the real call and a re-entrant token would otherwise clobber it. `flags` bit 0 / bit 1 record the context being null before / after. `pc0` with `pc` also gives the engine's own length for each token, which is what an operand model is judged against. **`rec`, the index entry and `rec_hash` are v5 (2026-09-11) the record the PROGRAM COUNTER is in**, found by scanning the live index for the last entry at or below `pc0 - 1` and requiring that entry to reach it -- the same question `hook.c`'s `find_record` answers, and the reason the two can now be checked against each other at all. Through v4 the id came from `ds:RECID`, an engine global written on load: on the 2026-09-11 Roppongi session that produced 165 false findings, 450 events reported unverified and 611 served events charged to the wrong record. `flags` bit 3 (`REC_FROM_PC`) says the scan found the record; without it `rec` is still `ds:RECID`, so `decode` and `bases` keep an id. `flags` bit 2 (`VIRTUAL`) says `pc0` was at or above `image_end`, where it names no record -- idx and hash are logged as 0 and the new `u16 handle` (`[ctx+0x0A]`, record offset +28) is what attributes it offline. The test is `pc0 >= image_end`, not `pc0 - 1`: the handoff to a record's first virtual tail lands on `image_end` exactly, and 2,429 of that session's events sit there. v1 (headerless, 16-byte), v2 (20-byte), v3 (22-byte) and v4 (28-byte) traces still decode; `giten trace verify` needs v4 or better. |
...
| overlay hook `.ovl` | 37 B | 2560 B | yes |

The `.ovl` figure is the cave, and it tracks the hook: 2048 when this table was
written, 4608 for overlay v5's per-span verification bitmaps, 5120 on
2026-09-10 for the merged-buffer fix, and **2560 the same day for v6**. The
halving is the point -- content addressing deleted the per-file directory, the
index fingerprint, the `(handle, fid)` cache, both verification bitmaps,
`entry_fits`, `merged_fetch` and the merge windows, and replaced them with two
binary searches and a 16-slot memo.

**The in-place count is still five `E8` call sites redirected and nothing else
of the original rewritten.** It reads 37 rather than 38 because the cave now
sits at a different address, so one byte of one rel32 displacement happens to
equal the byte that was already there. No new site, no new edit; `database` and
`mapnames` move by a byte or two for the same reason, since their patch
addresses are derived from the image size. `EXE_PASSES` in `tests/test_v2.py`
pins every pass's footprint, so a pass that starts editing the original in place
fails there.
