...
| dev | 0x449A30 | `ff1550404600` (`call dword ptr [gdi32!TextOutA]`) | `E8 <.tlg thunk> 90` | **TextOutA logger** (the menu-overlay spike). `TextOutA` is the only text-drawing API the exe imports and this is the only instruction that calls it, so every string GDI draws passes one point -- narrower than the overlay's five byte-fetch sites. The thunk records `u16 n, i16 x, i16 y` plus the cp932 bytes to `ddswin	extout.bin` behind a `"GTXT" u16 1 u16 0` header, then tail-*jumps* to the real function through the IAT slot so the `__stdcall` cleanup is untouched. **The import slot itself cannot be patched on disk** -- the loader overwrites `FirstThunk` with the resolved address at load time -- which is why the call site is what moves. Read it with `giten trace textout`. |
