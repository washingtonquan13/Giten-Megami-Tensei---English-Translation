...
| dev | 0x4516E0, 0x4517E7, 0x4518EC, 0x451A1F, 0x451C24, 0x451D28 | `E8 <draw glyph 0x451230>` | `E8 <.tlg thunk>` | **Glyph logger** (the menu-overlay spike). **The exe never calls `TextOutA`** -- a play session with a hook on it produced no log at all. It imports `GetGlyphOutlineA` and blits every character itself: `0x451230` draws one glyph, reading a built-in half-width font at `0x0046C230` and falling back to `GetGlyphOutlineA` (its only call site, `0x4512B6`) for the rest, and six draw-string variants (`0x451650`, `0x451750`, `0x451850`, `0x451950`, `0x451B20`, `0x451CB0`) walk a `char*` calling it per character. Those six are the level a menu overlay would hook, since each takes the string as an argument. This build redirects the six `call 0x451230` sites to a thunk that logs `u32 arg1..arg5` to `ddswin	extout.bin` behind a `"GTXT" u16 2 u16 20` header and tail-jumps to the real routine -- six rel32 operands, no trampolines. Read it with `giten trace textout`. |
