...
#: **Retired 2026-09-11.**  ``STRADDLE_MAX`` and ``tokenize_straddling`` lived
#: here.  They existed because the tokenizer walked a record in isolation, so a
#: token whose operands sit in the next record looked like a tiling failure and
#: had to be recovered with a bounded lookahead and accepted only under a
#: shape test ("every token but the last ends inside the record").  None of that
#: is needed now: :func:`giten.vmops.tokenize_record` walks the container image
#: the engine actually runs in, so such a token is an ordinary token and the
#: only thing left to record is how many bytes it reached past the record's own
#: end -- ``Rec.straddle``, which only the byte builder consults.
STRADDLE_RETIRED = "giten.vmops.tokenize_record"
