    ...
    for t in toks:
        if t.kind == "text":
            ...
            try:
                ...
                # cp932 has duplicate encodings (NEC row 89-92 vs the IBM
                # extension): FA 82 decodes to 冾 but re-encodes as ED 65.
                # A token that cannot reproduce its own bytes is shown raw so
                # the round trip stays exact (one span in the original).
                if text.encode("cp932") != raw:
                    raise UnicodeDecodeError("cp932", raw, 0, len(raw), "not round-trippable")
